...
function fn_update_product_user_price($product_id, $user_prices)
{
  //  delete old data
  ...
  foreach ($user_prices as $user_price) {
    ...
    try {
      ...
      if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
        fn_set_notification('W', __('warning'), __(
          'user_price__error__dublicate_entry',
          array(
              '%USER_EMAIL%' => db_get_field("SELECT email FROM ?:users WHERE user_id = ?s", $user_price['user_id'])
          ))
        );
      } else {
        throw $e;
