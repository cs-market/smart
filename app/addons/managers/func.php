...
function fn_managers_get_user_info($user_id, $get_profile, $profile_id, &$user_data) {
    ...
    if (SiteArea::isAdmin(AREA)) $user_data['managers'] = fn_get_managers(['user_managers' => $user_id]);
}

function fn_managers_user_roles_get_list(&$roles, $type) {
    if (empty($type) || $type != 'C') $roles['M'] = 'manager';
}
...
function fn_managers_get_order_info(&$order, $additional_data) {
    if (!empty($order) && SiteArea::isAdmin(AREA) && Registry::get('runtime.mode') == 'details' && UserRoles::is_manager()) {
...
function fn_update_user_managers($user_id, $managers) {
    db_query("DELETE FROM ?:user_managers WHERE user_id = ?i", $user_id);
    ...
    if (!empty($managers)) {
        ...
        if (!is_array($managers)) {
            ...
            $managers = array_column($managers, 'user_id');
        }
        ...
        foreach ($managers as $m_id) {
            if (!empty($m_id)) $udata[] = ['manager_id' => $m_id, 'user_id' => $user_id];
        }
...
function fn_managers_update_user_profile_pre($user_id, &$user_data, $action = '') {
    if (SiteArea::isAdmin(AREA) && !empty($user_data['user_id']) && isset($user_data['managers']) && $user_data['user_type'] == 'C') {
        fn_update_user_managers($user_id, $user_data['managers']);
    }
}
...
function fn_managers_place_order($order_id, $action, $order_status, $cart, $auth) {
    ...
    if (db_get_field("SELECT $field FROM ?:companies WHERE company_id = ?i", $order_info['company_id']) == 'Y') {
        ...
        $managers = fn_get_managers(['user_managers' => $order_info['user_id']]);
...
function fn_managers_get_tickets_params(&$params, $condition, $join) {
    if (ACCOUNT_TYPE == 'vendor' && !UserRoles::is_manager()) {
...
function fn_managers_send_form(&$page_data, $form_values, $result, $from, $sender, $attachments, $is_html, $subject) {
    if (Tygh::$app['session']['auth']['user_id']) {
        $managers = fn_get_managers(['user_managers' => Tygh::$app['session']['auth']['user_id']]);
        if (!empty($managers)) {
