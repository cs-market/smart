    ...
    <div class="ty-breadcrumbs clearfix">
        {strip}
            {foreach from=$breadcrumbs item="bc" name="bcn" key="key"}
                ...
                {/if}
                {if $bc.link}
                    <a href="{$bc.link|fn_url}" class="ty-breadcrumbs__a{if $additional_class} {$additional_class}{/if}"{if $bc.nofollow} rel="nofollow"{/if}>{if $smarty.foreach.bcn.first}<i class="ty-azure-icon-home"></i>{else}{$bc.title|strip_tags|escape:"html" nofilter}{/if}</a>
                {else}
