...
<div class="items-container {if $ability_sorting}cm-sortable  ui-sortable{/if}" id="malfunction_types_list"
...
<div class="table-responsive-wrapper">
    ...
    {foreach from=$statuses item="s" key="key"}
        ...
        {include file="common/object_group.tpl"
            id=$s.status|lower
            text=$s.description
            href="malfunction_types.update?status=`$s.status`&type=`$type`"
            href_delete=$cur_href_delete delete_target_id="malfunction_types_list,actions_panel"
...
{capture name="adv_buttons"}
    ...
    {capture name="add_new_picker"}
        {include file="addons/equipment/views/malfunction_types/update.tpl" status_data=[]}
    {/capture}
