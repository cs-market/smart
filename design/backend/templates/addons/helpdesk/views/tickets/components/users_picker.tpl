...
{if $view_mode != "list"}
	...
	{if !$no_container}<div class="buttons-container">{/if}{if $picker_view}[{/if}
		{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="tickets.picker?display=`$display`&amp;extra=`$extra_var`&amp;picker_for=`$picker_for`&amp;data_id=`$data_id`"|fn_url but_text=$_but_text but_role="add" but_rev="content_`$data_id`" but_meta="cm-dialog-opener"}
