	...
	{capture name="add_buttons"}
		{if $view_mode != "list"}
			...
			{if !$runtime.company_id || $runtime.controller != "companies"}
				...
				{if $_but_role != "icon"}
					...
					{/if}
				{/if}
				<div class="hidden" id="content_{$data_id}" title="{$but_text|default:__("add_stickers")}"></div>
			{/if}
		...
		{else}
			...
			{if !$runtime.company_id || $runtime.controller != "companies"}
				...
				{assign var="_but_role" value="add"}
				{assign var="_but_icon" value="icon-plus"}
			{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="stickers.picker?display=`$display`&root=`$default_name`&data_id=`$data_id`&company_ids=`$company_ids``$extra_url`"|fn_url but_text=$_but_text but_role=$_but_role but_icon=$_but_icon but_meta=$but_ but_target_id="content_`$data_id`" but_meta="`$but_meta` cm-dialog-opener"}
...
{if !$extra_var && $view_mode != "button"}
	...
	{else}
		<div id="{$data_id}" class="{if $multiple && !$item_ids}hidden{elseif !$multiple}{if $view_mode != "list"}cm-display-radio{/if}{/if} choose-sticker">
	{/if}
	{if $multiple}
		<tr class="hidden">
	...
	{/if}
		{if $multiple}		
			{include file="addons/product_stickers/pickers/stickers/js.tpl" sticker_id="`$ldelim`sticker_id`$rdelim`" holder=$data_id hide_input=$hide_input input_name=$input_name radio_input_name=$radio_input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button position_field=$positions position="0"}
		{/if}
		{if $view_mode == "list"}
			{foreach from=$item_ids item="s_id" name="items"}
				{include file="addons/product_stickers/pickers/stickers/js.tpl" sticker_id=$s_id holder=$data_id hide_input=$hide_input input_name=$input_name clone=true hide_link=$hide_link first_item=$smarty.foreach.items.first view_mode="list" show_only_name=true}
			{foreachelse}
				{include file="addons/product_stickers/pickers/stickers/js.tpl" sticker_id="" holder=$data_id hide_input=$hide_input input_name=$input_name clone=true hide_link=$hide_link view_mode="list"}
			{/foreach}
		{else}
			{foreach from=$item_ids item="c_id" name="items"}
				{if !$multiple}<div class="input-append choose-input">{/if}
				{include file="addons/product_stickers/pickers/stickers/js.tpl" sticker_id=$c_id holder=$data_id hide_input=$hide_input input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration+$start_pos}
				{if !$multiple}</div>{/if}<!-- /.choose-input -->
			{foreachelse}
				{if !$multiple}
				<div class="input-append choose-input">
					{include file="addons/product_stickers/pickers/stickers/js.tpl" sticker_id="" holder=$data_id hide_input=$hide_input input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button}
					{$smarty.capture.add_buttons nofilter}
				</div>
				{/if}
			{/foreach}
