...
<tr>
  ...
  <td data-th="{__("quantity")}" class="center order-management-quantity">
    ...
    <span class="cm-reload-{$key}" id="amount_update_{$key}">
      ...
      {/if}
      <input type="text" size="3" id="amount_{$key}" name="cart_products[{$key}][amount]" value="{$cp.amount}" class="input-micro cm-amount" {if $cp.qty_step> 1} data-ca-step="{$cp.qty_step}"{/if} />
      {if $settings.Appearance.quantity_changer == "Y"}
