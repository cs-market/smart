...
<form name="profile_form" enctype="multipart/form-data" action="{""|fn_url}" method="post" class="form-horizontal form-edit form-table {if 0 && ($runtime.company_id && $id && $user_data.company_id != $runtime.company_id && $id != $auth.user_id) || $hide_inputs} cm-hide-inputs{/if}">
...
{capture name="tabsbox"}
    ...
    {if !"ULTIMATE:FREE"|fn_allowed_for}
        {if $id && (
            ((!$user_data|fn_check_user_type_admin_area || !$user_data.user_id)) 
            || ($user_data|fn_check_user_type_admin_area && $user_data.user_id && $usergroups && !$runtime.company_id && ($user_data.company_id != 0 || ($user_data.company_id == 0 && $user_data.is_root != 'Y'))) 
            || ($user_data.user_type == 'V' && $runtime.company_id && $auth.is_root == 'Y' && $user_data.user_id != $auth.user_id && $user_data.company_id == $runtime.company_id))
        }
                    ...
                    {foreach from=$usergroups item=usergroup}
                        <tr>
                            ...
                            <td class="right" data-th="{__("status")}">
                                ...
                                {/if}
                                {include file="common/select_popup.tpl" id=$usergroup.usergroup_id status=$ug_status hidden="" items_status="profiles"|fn_get_predefined_statuses extra="&user_id=`$id`" update_controller="usergroups" notify=true hide_for_vendor=false}
                            </td>
                        </tr>
                    {/foreach}
                    </table>
