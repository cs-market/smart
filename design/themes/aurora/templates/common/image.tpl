...
{elseif $show_no_image}
    <span class="ty-no-image" style="--aspect-ratio: {($image_height/$image_width)|round:2}; height: {$image_height|default:$image_width}px; width: {$image_width|default:$image_height}px; ">{include_ext file="common/icon.tpl" class="ty-icon-image ty-no-image__icon" title=__("no_image")}</span>
{/if}
{if $show_detailed_link}
