{hook name="products:qty"}
    ...
    <div class="ty-qty clearfix{if $settings.Appearance.quantity_changer == "Y"} changer{/if}" id="qty_{$obj_prefix}{$obj_id}">
        ...
        {/if}
        <input type="text" size="5" class="ty-value-changer__input cm-amount" id="qty_count_{$obj_prefix}{$obj_id}" name="product_data[{$obj_id}][amount]" value="{$default_amount}" data-ca-val="{$default_amount}" {if $product.qty_step} data-ca-step="{$product.qty_step}"{/if} data-ca-min-qty="0{*if $product.min_qty}{$product.min_qty}{/if*}" />
        {if $settings.Appearance.quantity_changer == "Y"}
