...
<a id="sw_elm_pagination_steps" class="ty-dropdown-box__title cm-combination">{$pagination.items_per_page} {__("per_page")}<i class="ty-sort-dropdown__icon ty-icon-down-micro"></i></a>
    <ul id="elm_pagination_steps" class="ty-dropdown-box__content cm-popup-box hidden">
        {foreach from=$product_steps item="step"}
        {if $step != $pagination.items_per_page}
            <li class="ty-dropdown-box__item">
                <a class="{$ajax_class} ty-dropdown-box__item-a" href="{"`$range_url`&items_per_page=`$step`"|fn_url}" data-ca-target-id="{$pagination_id}" rel="nofollow">{$step} {__("per_page")}</a>
            </li>
        {/if}
        {/foreach}
    </ul>
