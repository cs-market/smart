        ...
        <a href="{"checkout.cart"|fn_url}" class='azure-indicator-container ty-icon-menu'>
            {hook name="checkout:dropdown_title"}
                {if $smarty.session.cart.amount}
                    ...
                    {if $smarty.session.cart.amount}
                        <div class='azure-count-indication'><div>{$smarty.session.cart.amount}</div></div>
                    {/if}
                {else}
                    ...
                    {/if}
                {/if}
                <span class="cart-text">{if $smarty.session.cart.amount}{include file="common/price.tpl" value=$smarty.session.cart.total span_id="total_price_`$key`_`$dropdown_id`" class="none"}{else}{__("cart")}{/if}</span>
