...
<div class="ty-product-filters__horizontal-wrapper">
{if $items}
...
{foreach from=$items item="filter" name="filters"}
    ...
    <div class="ty-product-filters__block">
        {if $items|count-1}<span class="ty-product-filters__title">{$filter.filter}{if $filter.selected_variants} {*({$filter.selected_variants|sizeof})*}{/if}{if $reset_url}<a class="cm-ajax cm-ajax-full-render cm-history" href="{$reset_url|fn_url}" data-ca-event="ce.filtersinit" data-ca-target-id="{$ajax_div_ids}" data-ca-scroll=".ty-mainbox-title"><i class="ty-icon-cancel-circle"></i></a>{/if}</span>{/if}
        {if $filter.slider}
            ...
            {/if}
        {else}
            {include file="addons/smart_distribution/blocks/product_filters/components/product_horizontal_filter_variants.tpl" filter_uid=$filter_uid filter=$filter collapse=$collapse}
        {/if}
    </div>
    {/hook}
{/foreach}
...
<div class="ty-product-filters__tools clearfix">
    ...
    <a href="{$filter_base_url|fn_url}" rel="nofollow" class="ty-product-filters__reset-button cm-ajax cm-ajax-full-render cm-history" data-ca-event="ce.filtersinit" data-ca-scroll=".ty-mainbox-title" data-ca-target-id="{$ajax_div_ids}"><i class="ty-product-filters__reset-icon ty-icon-cw"></i> {__("all")}</a>
