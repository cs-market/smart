...
<div class="ty-reward-points__summary">
    {if $nearest_expiration}
        <br>{__('userlog_expiration_text', ['[amount]' => $nearest_expiration.amount, '[ttl]' => $nearest_expiration.ttl|date_format:"`$settings.Appearance.date_format`"])}
    {/if}
</div>
...
<thead>
    <tr>
        ...
        <th class="ty-reward-points-userlog__points_expitation">{__("userlog_expiration_date")}</th>
...
<tr>
    ...
    <td>{$ul.amount}</td>
    <td>{if $ul.expiration.ttl}{$ul.expiration.ttl|date_format:"`$settings.Appearance.date_format`"}{/if}</td>
    <td>
...
{capture name="mainbox_title"}{__("reward_points_log")}: {__('points_lowercase', [$auth.points])}{/capture}
